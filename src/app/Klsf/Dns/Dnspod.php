...
class Dnspod implements DnsInterface
{
    ...
    public function updateDomainRecord($RecordId, $Name, $Type, $Value, $LineId, $DomainId = null, $Domain = null, $Options = null)
    {
        $params = [];
        $DomainId && $params['domain_id'] = $DomainId;
        $Domain && $params['domain'] = $Domain;
        ...
        // 处理MX记录优先级
        if ($Type === 'MX') {
            if ($Options && isset($Options['mx'])) {
                $params['mx'] = intval($Options['mx']);
            } else {
                $params['mx'] = 10; // 默认值
            }
        }
        ...
        if ($Type === 'SRV' && $Options) {
            if (isset($Options['priority'])) {
                $params['priority'] = intval($Options['priority']);
            }
            if (isset($Options['weight'])) {
                $params['weight'] = intval($Options['weight']);
            }
            if (isset($Options['port'])) {
                $params['port'] = intval($Options['port']);
            }
        }
    ...
    public function addDomainRecord($Name, $Type, $Value, $LineId, $DomainId = null, $Domain = null, $Options = null)
    {
        $params = [];
        $DomainId && $params['domain_id'] = $DomainId;
        $Domain && $params['domain'] = $Domain;
